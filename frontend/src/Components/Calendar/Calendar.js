...
function Calendar(props) {
  ...
  const handleSave = (e) => {
    ...
    if (e.requestType === 'eventCreate') {
      ...
      axios
        .post(`http://localhost:8000/api/event`, body, config)
        .then((res) => {
    ...
    if (e.requestType === 'eventRemove') {
      axios
        .get(
          `http://localhost:8000/api/event/${e.deletedRecords[0]._id}`,
          config
        )
        .then((res) => {
    ...
    if (e.requestType === 'eventChange') {
      ...
      axios
        .post(
          `http://localhost:8000/api/event/update/${id}`,
          body,
          config
        )
        .then((res) => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`http://localhost:8000/api/event`, config)
      .then((res) => {
